...
class CameraViewController: UIViewController {
  ...
  private var lastDetector: Detector?
    ...
    private var mouthThreshold: Int = 5
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
      mouthThreshold = UserDefaults.standard.integer(forKey: "detectionSensitivity")
      if mouthThreshold == 0 {
          mouthThreshold = 5
      }
      print(mouthThreshold)
  ...
  @IBAction func switchOverlay(_ sender: UIButton) {
    ...
    @IBAction func toggleRecording(_ sender: UIButton) {
        ...
        if isRecording {
            ...
            data = [Int: [String: Any]]()
            curTS = 0
            SVProgressHUD.showSuccess(withStatus: "Recording Started")
        }else{
            var str: [String] = ["Timestamp,Meal Type,Food Type,Mouth Distance of Opening,Mouth Movement,Left Shoulder Coordinate,Left Wrist Coordinate,Left Elbow Coordinate,Left Hand Movement,Right Shoudler Coordinate,Right Wrist Coordinate,Right Elbow Coordinate,Right Hand Movement"]
            let sortedKey = Array(data.keys).sorted(by: <)
            
            for t in sortedKey {
                if
                    let item = data[t],
                    let mt = UserDefaults.standard.string(forKey: "mealType"),
                    let ft = UserDefaults.standard.string(forKey: "foodType"),
                    ...
                    str.append("\(t),\(mt),\(ft),\(md),\(m),\(ls),\(lw),\(le),\(l),\(rs),\(rw),\(re),\(r)")
                }
            ...
            do {
                ...
                SVProgressHUD.showSuccess(withStatus: "Saving Failed")
            }
...
//        print("Pose detector returned no results.")
  ...
  private func addContours(for face: Face, width: CGFloat, height: CGFloat) {
    ...
    averageLowerLipY /= bottomLipCount
      ...
      if abs(Int(averageLowerLipY) - Int(averageUpperLipY)) <= mouthThreshold {
          if isRecording {
